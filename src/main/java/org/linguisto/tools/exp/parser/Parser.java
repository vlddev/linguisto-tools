...
public class Parser {
    ...
    public static String getParsedTranslation(String strTr) {
        // replace links like [[]] with <kref>
        // parse comments in round brackets
        StringBuffer sb = new StringBuffer();
        for (SChunk chunk : Parser.parseTranslation(strTr)) {
            if (chunk.getStyleBegin() != null) {
                sb.append(chunk.getStyleBegin());
            }
            if (chunk.isLink()) {
                sb.append("<kref>").append(chunk.getLinkId()).append("</kref>");
            } else { //text
                sb.append(chunk.getText());
            }
            if (chunk.getStyleEnd() != null) {
                sb.append(chunk.getStyleEnd());
            }
        }
        return sb.toString();
    }
    ...
    public static String getParsedExample(String strTr) {
        StringBuffer sb = new StringBuffer();
        for (SChunk chunk : Parser.parseExample(strTr)) {
            if (chunk.getText() != null && chunk.getText().length() > 0) {
